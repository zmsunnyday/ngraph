...
It's possible to derive a trainable model from any graph that has been 
constructed with weight-based updates. For this example named ``mnist_mlp.cpp``, 
we start with a hand-designed inference model and convert it to a model that 
can be trained with nGraph. 
...
Repeat the process for the next layer, which gets normalized with 
a ``softmax``.
...
Next, a function representing :term:`cross-entropy` is needed to compute 
the loss. The nGraph library does not currenty have a standardized core 
``Op`` for cross-entropy, so we'll implement something that approximates 
our estimate "cost" directly, and add clipping to prevent underflow.
...
To produce the updates, a computation is needed to get an adjustment 
for the weights *from* an adjustment to the loss. In nGraph, ``loss`` 
is the computation that computes the loss; it is equivalent
to what some descriptions of the autodiff algorithm call "the tape".

Since each step of a computation between a weight and the loss has a
derivative, it makes sense to use the reverse-mode autodiff to compute 
an update for the weight from an update for the loss. In fact, this same
method can be used to compute updates for all the weights, by sharing 
much of the update computation; this is what some frameworks do. However, 
it is just as easy to instead create the update computations for all of 
the weights, permitting compilation to optimize across steps in the 
computation.

Let's call the adjustment to the loss
...
The different update nodes will share intermediate computations. 
So to get the updated value for ``W0``, try
