...
void ngraph::runtime::cpu::pass::CPUFusion::construct_conv_bias_strided()
{
    Shape shape{2, 2, 1, 1};
    auto data_batch = std::make_shared<pattern::op::Label>(element::f32, shape);
    auto filters = std::make_shared<pattern::op::Label>(element::f32, shape);
    ...
    auto pbroadcast = std::make_shared<op::Broadcast>(pbias, shape, AxisSet{0, 1, 2, 3});
    auto pmaxpool = std::make_shared<op::MaxPool>(pbroadcast, Shape{1, 1}, Strides{2, 2});
    ...
    auto pconv1 = std::make_shared<op::Convolution>(data_batch,
                                                    ...
                                                    CoordinateDiff{0, 0},
                                                    Strides{1, 1});
    auto p_conv_bias = pmaxpool + pconv1;
    ...
    ngraph::pattern::graph_rewrite_callback callback = [](pattern::Matcher& m) {
        ...
        if (conv->get_input_shape(0).size() == 4)
        {
            auto bias = m.get_match_root()->get_argument(1)->get_argument(0)->get_argument(0);
            auto bias_shape = bias->get_shape();
            if (bias_shape.size() > 1)
            {
                NGRAPH_DEBUG
...
void ngraph::runtime::cpu::pass::CPUFusion::construct_conv_add()
{
    ...
    auto padd = std::make_shared<op::Add>(pconv, add_input);

    pattern::graph_rewrite_callback callback = [data_batch, filters, add_input](
        pattern::Matcher& m) {
        ...
        auto conv_add = std::shared_ptr<Node>(new op::ConvolutionAdd(conv, pattern_map[add_input]));
        ngraph::replace_node(m.get_match_root(), conv_add);
        return true;
    };

    auto m = std::make_shared<pattern::Matcher>(padd, callback);
    this->add_matcher(m);
}
...
void ngraph::runtime::cpu::pass::CPUFusion::construct_conv_bias_add()
{
    ...
    auto prelu = std::make_shared<op::Relu>(padd);
    ...
    pattern::graph_rewrite_callback callback =
        [data_batch, filters, add_input](pattern::Matcher& m) {
            ...
            auto pattern_map = m.get_pattern_map();
            auto conv = std::dynamic_pointer_cast<op::ConvolutionBias>(
                m.get_match_root()->get_argument(0)->get_argument(1));

            if (!conv)
            {
                conv = std::dynamic_pointer_cast<op::ConvolutionBias>(
            ...
            auto conv_add =
                std::shared_ptr<Node>(new op::ConvolutionBiasAdd(conv, pattern_map[add_input]));
            ngraph::replace_node(m.get_match_root(), conv_add);
            return true;
        };
