...
void ngraph::runtime::cpu::pass::CPUFusion::construct_conv_bias_bprop()
{
    ...
    ngraph::pattern::graph_rewrite_callback callback = [data_batch, delta](pattern::Matcher& m) {
        ...
        auto sum = std::make_shared<op::Sum>(pattern_map[delta], AxisSet{0, 1, 2, 3});
       
        for (auto delta_user: pattern_map[delta]->get_users())
        {
            if (delta_user->is_same_op_type(sum))
            {
               ...
               //for (descriptor::Input& input : delta_user->get_outputs().at(0).get_node()->get_inputs())
               //   input.replace_output(goe2, 0);
