...
std::string file_util::get_file_name(const std::string& s)
{
    string rc = s;
...
std::string file_util::get_file_ext(const std::string& s)
{
...
std::string file_util::get_temp_directory()
{
...
std::string file_util::read_file_to_string(const std::string& path)
{
    std::ifstream f(path);
    std::stringstream ss;
    ss << f.rdbuf();
...
void file_util::iterate_files_worker(const string& path,
                                     std::function<void(const string& file, bool is_dir)> func,
                                     bool recurse)
{
    DIR* dir;
...
void file_util::touch(const std::string& filename)
{
...
bool file_util::exists(const std::string& filename)
{
    struct stat buffer;
    return (stat(filename.c_str(), &buffer) == 0);
}

int file_util::try_get_lock(const std::string& filename)
{
    mode_t m = umask(0);
...
void file_util::release_lock(int fd, const std::string& filename)
{
    if (fd >= 0)
    {
...
time_t file_util::get_timestamp(const std::string& filename)
{
    time_t rc = 0;
    struct stat st;
